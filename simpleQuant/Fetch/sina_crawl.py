...
def get_east_fund_fs(uri):   
   req=urlopen(uri,timeout=60)
   lines=req.read().decode() 
   req.close()
   rst=re.findall('[\[](.*?)[\]]',lines)[0]
   ax=rst.replace("-,","0.00,").replace('-"','0.00"')[1:-1].split('","')   
   return pd.DataFrame((x.split(',') for x in ax), dtype=float,\
             columns=['r0_net','r1_net','r2_net','r3_net','r4_net'])


fszj="http://ff.eastmoney.com/EM_CapitalFlowInterface/api/js?type=ff\
&check=MLBMS&cb=&js=&rtntype=3&acces_token=1942f5da9b46b069953c873404aad4b5&_=1535450561"
for a in cc:
     print(fszj+a)
     rst=get_east_fund_fs(fszj+str(random.randrange(100,999,2))+"&id="+a)
     rst['code']=a
     tmp=rst.join(tt)
     ll.append(tmp)
     time.sleep(5)
